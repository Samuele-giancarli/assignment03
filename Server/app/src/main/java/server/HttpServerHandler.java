...
class HttpServerHandler implements HttpHandler {
    ...
    private String createResponseFromQueryParams(URI uri) {
                    ...
                    if (param.length > 0) {
                        for (int i = 0; i < param.length; i++) {
                            if (LEVEL.equalsIgnoreCase(param[PARAM_NAME_IDX])) {
                                passedParam = param[PARAM_VALUE_IDX];
                            }
                            if (STATE.equalsIgnoreCase(param[PARAM_NAME_IDX])) {
                                passedParam = param[PARAM_VALUE_IDX];
                            }
                            if (MODE.equalsIgnoreCase(param[PARAM_NAME_IDX])) {
                                passedParam = param[PARAM_VALUE_IDX];
                            }
                            if (OPENING.equalsIgnoreCase(param[PARAM_NAME_IDX])) {
                                passedParam = param[PARAM_VALUE_IDX];
                            }
                        }
                    }
        ...
        //this is what determine the output
        return "hello" + passedParam;
